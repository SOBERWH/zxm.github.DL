...
def trainer(ops,f_log):
    try:
        ...
        # 断点续训：加载checkpoint
        start_epoch = 0
        if ops.resume and os.path.exists(ops.resume):
            checkpoint = torch.load(ops.resume, map_location=device)
            model_.load_state_dict(checkpoint['model_state_dict'])
            optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
            start_epoch = checkpoint['epoch'] + 1
            best_loss = checkpoint.get('best_loss', np.inf)
            init_lr = checkpoint.get('learning_rate', ops.init_lr)
            set_learning_rate(optimizer, init_lr)
            print('Resume training from epoch {} (best_loss: {:.6f})'.format(start_epoch, best_loss))
        ...
        for epoch in range(start_epoch, ops.epochs):
            if ops.log_flag:
                sys.stdout = f_log
            print('\nepoch %d ------>>>'%epoch)
            model_.train()
            ...
            checkpoint = {
                ...
                'model_exp': ops.model_exp
            }
            torch.save(checkpoint, ops.model_exp + 'checkpoint_epoch-{}.pth'.format(epoch))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--resume', type=str, default = None,
        help = 'resume_checkpoint') # 断点续训checkpoint路径
